...
getCastList().then((castList) => {
    console.log(castList);
});

if(!fs.existsSync(path.dirname(process.execPath) + '/settings.json')){
    fs.writeFileSync('./settings.json',JSON.stringify({}));
    console.log('settings.jsonを作成しました。');
}

const settings = require(path.dirname(process.execPath) + '/settings.json');
...
async function getYomiageVoiceStream(text,character,speed,filename){
    return new Promise((resolve, reject) => {
        let voice = getWave(text, character, speed, path.join(path.dirname(process.execPath), filename + '.wav'))
            if(!voice){
                reject('voice is false');
            }
            const stream = fs.createReadStream(path.join(path.dirname(process.execPath), filename + '.wav'));
            resolve(stream);
    });
}
...
client.on('messageCreate', async (message) => {
    ...
    if(settings[message.guildId] !== undefined){
